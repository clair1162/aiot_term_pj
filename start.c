...
int main() {
	...
	pid = fork();
	if(pid > 0) {
		thr_id = pthread_create(&fnd_thread, NULL, FndThread, (void *)&a);

		if(thr_id < 0) {
			perror("thread create error: ");
			exit(1);
		}
		pthread_join(fnd_thread, (void **)&status);
		...
		thr_id = pthread_create(&choice_thread, NULL, IPCThread, (void *)&b);
		...
		pthread_join(fnd_thread, (void **)&status);
		pthread_join(choice_thread, (void **)&status);
